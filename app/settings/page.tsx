...
import { Save, ArrowLeft, DollarSign, Mail, Linkedin, Key, CheckCircle2, XCircle, Loader2, AlertTriangle, Eye, EyeOff, Trash2 } from "lucide-react";
...
export default function SettingsPage() {
    const { channelSpend, setChannelSpend } = useSettingsStore();
    ...
    return (
        <main className="min-h-screen bg-background">
            <div className="mx-auto max-w-2xl px-6 py-10">
                ...
                <Link
                    href="/"
                    className="mb-6 inline-flex min-h-[2.75rem] items-center gap-2 rounded-lg px-3 py-2 text-sm text-text-muted transition-colors hover:bg-surface hover:text-text-primary"
                >
                    <ArrowLeft size={16} />
                    Back to Dashboard
                </Link>
                ...
                <div className="mb-8">
                    ...
                    <p className="mt-1 text-sm text-text-muted">
                        Connect your platforms and configure dashboard settings.
                    </p>
                </div>
                ...
                <div className="rounded-xl border border-border bg-surface p-6">
                    ...
                    <p className="mb-6 text-xs text-text-muted">
                        Enter your monthly spend for each outbound channel. These values are used to compute cost per meeting and
                        overall ROI on the dashboard.
                    </p>
                    ...
                    <div className="space-y-5">
                        ...
                        <div>
                            <label
                                htmlFor="instantly-spend"
                                className="mb-2 flex items-center gap-2 text-sm font-medium text-text-secondary"
                            >
                                <div className="flex h-6 w-6 items-center justify-center rounded bg-instantly/15 text-instantly">
                                    <Mail size={14} />
                                </div>
                            ...
                            <p className="mt-1 text-xs text-text-muted">
                                Current: {formatCurrency(channelSpend.instantly)} / month
                            </p>
                        </div>
                        ...
                        <div>
                            <label
                                htmlFor="heyreach-spend"
                                className="mb-2 flex items-center gap-2 text-sm font-medium text-text-secondary"
                            >
                                <div className="flex h-6 w-6 items-center justify-center rounded bg-heyreach/15 text-heyreach">
                                    <Linkedin size={14} />
                                </div>
                            ...
                            <p className="mt-1 text-xs text-text-muted">
                                Current: {formatCurrency(channelSpend.heyreach)} / month
                            </p>
                        </div>
                    </div>
                ...
                {/* Info */}
                <div className="mt-6 rounded-xl border border-border-subtle bg-surface/50 p-5">
                    <h3 className="mb-2 text-sm font-semibold text-text-primary">How ROI is Calculated</h3>
                    <ul className="space-y-1 text-xs text-text-muted">
                        <li>• <strong>Opportunities</strong> = Meetings Booked × 35% conversion rate</li>
                        <li>• <strong>Pipeline Value</strong> = Opportunities × $5,000 avg deal value</li>
                        <li>• <strong>ROI</strong> = (Pipeline Value − Spend) ÷ Spend × 100</li>
                    </ul>
                </div>

                {/* Where to find API keys */}
                <div className="mt-4 rounded-xl border border-border-subtle bg-surface/50 p-5">
                    ...
                    <ul className="space-y-2 text-xs text-text-muted">
                        <li className="flex items-start gap-2">
                            ...
                            <span>
                                <strong className="text-text-secondary">Instantly.ai:</strong> Go to{" "}
                        ...
                        <li className="flex items-start gap-2">
                            ...
                            <span>
                                <strong className="text-text-secondary">HeyReach:</strong> Go to{" "}
